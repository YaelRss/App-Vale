<template>
  <div class="space-y-5 sm:space-y-8 max-w-6xl mx-auto pb-12">
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
      <div class="flex gap-2 overflow-x-auto no-scrollbar py-1 w-full sm:w-auto">
        <button 
          ...
          :class="[
            'px-4 py-2.5 rounded-2xl text-xs sm:text-sm font-bold transition-all duration-150 border whitespace-nowrap shrink-0 touch-manipulation active:scale-95',
    ...
    <!-- Buscador Espacioso con Icono SVG -->
    <div class="relative">
      ...
      <input 
        ...
        class="w-full pl-12 pr-12 py-3.5 sm:py-4 bg-white dark:bg-slate-900 border border-rose-100/90 dark:border-slate-800 rounded-3xl text-sm sm:text-base text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-400 transition-all shadow-xs"
      />
      <button 
        v-if="search" 
        @click="search = ''" 
        class="absolute right-4 top-4 text-slate-400 hover:text-rose-500 p-1 rounded-xl transition"
      >
    ...
    <div v-if="selectedDoc" class="bg-white dark:bg-slate-900 border border-rose-100/80 dark:border-slate-800 rounded-3xl overflow-hidden shadow-xs">
      <div class="p-5 sm:p-8 bg-rose-50/50 dark:bg-slate-950/80 border-b border-rose-100/80 dark:border-slate-800 flex flex-col md:flex-row justify-between md:items-center gap-3">
        <div>
          <span class="text-xs font-mono uppercase tracking-widest text-rose-500 font-bold">{{ selectedDoc.nif_reference }}</span>
          <h2 class="text-lg sm:text-2xl font-black text-slate-900 dark:text-slate-100 mt-1">{{ selectedDoc.title }}</h2>
          <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">{{ selectedDoc.description }}</p>
        </div>
      </div>

      <div class="p-4 sm:p-8 space-y-6 sm:space-y-8">
        <div v-for="section in filteredSections" :key="section.id" class="space-y-4">
          <h3 class="text-xs sm:text-sm font-black tracking-wider uppercase text-rose-950 dark:text-slate-400 flex items-center gap-2.5">
            <span class="w-2.5 h-2.5 rounded-full shrink-0" :class="getNatureBadge(section.nature)"></span>
          ...
          <div class="grid gap-3">
            <div 
              ...
              :key="item.id"
              class="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 rounded-2xl border transition-all duration-150 gap-3 sm:gap-4"
            ...
            >
              <div class="flex items-start gap-3 flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-2 shrink-0 mt-0.5 sm:mt-0">
                  <span v-if="item.code" class="px-2.5 py-1 bg-rose-100/70 dark:bg-slate-800 text-rose-900 dark:text-slate-300 rounded-lg text-xs font-mono font-bold">
                    {{ item.code }}
                  </span>
                  <span v-if="item.abbreviation" class="px-2.5 py-1 bg-pink-100 text-pink-700 dark:bg-pink-950/80 dark:text-pink-300 border border-pink-200 dark:border-pink-800/60 rounded-lg text-xs font-mono font-bold">
                    {{ item.abbreviation }}
                  </span>
                  <span v-if="item.operation_sign" class="w-6 h-6 flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-mono font-black">
                    {{ item.operation_sign }}
                  </span>
                </div>

                <div class="flex-1 min-w-0 space-y-1">
                  <div class="text-sm sm:text-base font-bold text-slate-800 dark:text-slate-200 flex flex-wrap items-center gap-2">
                    <span class="truncate">{{ item.name }}</span>
                    <span 
                      class="text-[10px] sm:text-xs px-2.5 py-0.5 rounded-full font-bold uppercase shrink-0"
                  ...
                  <p v-if="item.definition" class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{{ item.definition }}</p>
                  <p v-if="item.formula_expression" class="text-xs font-mono text-rose-500 dark:text-rose-400 pt-0.5 flex items-center gap-1.5">
                    <svg class="w-3.5 h-3.5 text-rose-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M7 7h10M7 12h10M7 17h10"/></svg>
